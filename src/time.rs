...
//!
//! Ported from `unidpp-core`'s `unidpp_model::time` (same doctrine):
//! hand-rolled on purpose — no external time dependency, exact integer
//! arithmetic (seconds since the UNIX epoch; sub-second precision is not
//! needed at the resolver), strict parsing, canonical display with the
//! trailing `Z`. All resolver stamps are UTC; local time is a
...
impl Timestamp {
    ...
    /// Current wall-clock time. Handlers prefer explicit `asof` values;
    /// this is only the default resolution instant (I13: every response
    /// is as-of stamped).
