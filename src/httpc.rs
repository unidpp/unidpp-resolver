...
impl Url {
    pub fn parse(s: &str) -> Result<Url, String> {
        ...
        if scheme.eq_ignore_ascii_case("https") {
            return Err(format!(
                "https upstream is not supported by the reference client: `{s}`"
            ));
        }
...
mod tests {
    ...
    #[test]
    fn url_parsing() {
        let u = Url::parse("http://127.0.0.1:8080/resolve?x=1").unwrap();
        assert_eq!(u.host, "127.0.0.1");
        assert_eq!(u.port, 8080);
        assert_eq!(u.path_and_query, "/resolve?x=1");
    ...
    fn query_encoding() {
        assert_eq!(
            Url::encode_query_component("gs1:(01)06901234567892(21)AB"),
            "gs1%3A%2801%2906901234567892%2821%29AB"
        );
